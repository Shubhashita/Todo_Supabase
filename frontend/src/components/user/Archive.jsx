...
const Archive = ({ notes, setNotes, onArchive, onDelete, settings, toggleSidebar, onSearch }) => {
    const [selectedNote, setSelectedNote] = useState(null);
    const [isMenuOpen, setIsMenuOpen] = useState(false);

    // Filter only archived notes that are not trashed
    ...
    return (
        <main className="flex-1 flex flex-col h-full relative bg-white/5 md:bg-white/10 backdrop-blur-2xl text-white font-sans overflow-hidden [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']">
            <div className="pt-4 px-4 md:pt-10 md:px-10 flex-shrink-0">
                <div className="flex items-center justify-between w-full md:w-[94%] mx-auto mb-4 md:mb-8">
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="text-3xl md:text-6xl font-black tracking-tighter text-white/90 uppercase">ARCHIVE</h1>
                    </div>
                ...
                <div className="w-full md:w-[94%] mx-auto py-4">
                    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                        {archivedNotes.length > 0 ? (
                            <SortableContext items={archivedNotes} strategy={rectSortingStrategy}>
                                <div className="flex flex-row gap-4 md:gap-6 items-start">
                                    {getMasonryColumns(archivedNotes).map((colNotes, colIndex) => (
                            ...
                            <div className="text-center text-white/50 text-2xl mt-20">No archived notes</div>
                        )}
                    </DndContext>
                </div>
            </div>

            {/* View Modal Overlay (Differs slightly: No editing title/content typically in archive, but we can allow or read-only) */}
            {selectedNote && (
                <div
                        ...
                        <div className="absolute top-6 right-6 z-30">
                            ...
                            {isMenuOpen && (
                                <div className="absolute right-0 top-full mt-2 w-48 bg-[#1e1e2f]/90 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl overflow-hidden animate-fade-in origin-top-right">
                                    ...
                                    >
                                        <FiRefreshCcw className="mr-3" /> Unarchive
                                    </button>
