...
import { FiPlus, FiTrash2, FiClock, FiCheckCircle, FiMoreVertical, FiArchive, FiArrowLeft, FiMenu, FiUser, FiXCircle } from 'react-icons/fi';
...
const Notes = ({ notes, setNotes, settings, labels, setLabels, onCreate, onUpdate, onDelete, onArchive, onPin, filterStatus, onCreateLabel, currentLabel, filterDate, toggleSidebar }) => {
    ...
    const handlePinWithConfirmation = (e, noteId, isPinned) => {
        ...
        if (window.confirm(`Are you sure you want to ${action} this note?`)) {
            ...
            onPin(noteId, newStatus);
            if (selectedNote && selectedNote.id === noteId) {
                setSelectedNote(prev => ({ ...prev, isPinned: newStatus }));
            }
        }
    };
    ...
    return (
        <main className="flex-1 flex flex-col h-full relative bg-white/5 md:bg-white/10 backdrop-blur-2xl text-white font-sans overflow-hidden">
            <div className="pt-4 px-4 md:pt-10 md:px-10 flex-shrink-0">
                <div className="flex items-center justify-between w-full md:w-[94%] mx-auto mb-4 md:mb-8">
                    <div className="flex items-center gap-3 md:gap-6">
                        ...
                        <h1 className="text-3xl md:text-6xl font-black tracking-tighter text-white/90">SLATE</h1>
                    </div>
                    <div className="flex items-center gap-2 md:gap-5">
                        <div className="flex items-center gap-2 md:gap-4 bg-white/10 md:bg-transparent p-1.5 md:p-0 rounded-full pr-4 md:pr-0">
                            ...
                            <span className="text-sm md:text-xl font-bold text-white/90 max-w-[80px] md:max-w-[150px] truncate">{username}</span>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                        <SortableContext items={activeNotes} strategy={rectSortingStrategy}>
                            {pinnedNotes.length > 0 && (
                                <div className="mb-12">
                                    <h2 className="text-sm md:text-xl font-bold text-white/50 uppercase tracking-[0.2em] mb-4 md:mb-6 flex items-center gap-3">
                                        <BsPinFill className="text-white/40" /> Pinned
                                    </h2>
                                            ...
                                            <div key={colIndex} className="flex flex-col gap-6 flex-1 min-w-0">
                                                {colNotes.map((note) => (
                                                    <SortableNote
                                                        ...
                                                        className="bg-white/5 backdrop-blur-md border border-white/20 rounded-2xl md:rounded-3xl h-[180px] md:h-[200px] p-4 md:p-6 transition-all duration-300 cursor-pointer relative overflow-hidden hover:-translate-y-1 md:hover:-translate-y-2 hover:scale-[1.01] md:hover:scale-[1.02] hover:bg-white/10 hover:border-white/40 hover:shadow-2xl flex flex-col group hover:z-10"
                                                    >
                                                        <div className="flex justify-between items-start mb-2 md:mb-3">
                                                            <h3 className="font-bold text-lg md:text-2xl truncate pr-6 flex-1 text-white">{note.title || 'Untitled'}</h3>
                                                            {note.isPinned && <BsPinFill className="text-white shrink-0 ml-2 animate-pulse" size={16} />}
                                                        </div>
                                                        <div className="flex-1 overflow-hidden relative">
                                                            <p className="text-base md:text-xl text-white/90 whitespace-pre-wrap leading-relaxed font-medium mb-3 md:mb-4">
                                    ...
                                    <div key={colIndex} className="flex flex-col gap-6 flex-1 min-w-0">
                                        {colNotes.map((note) => (
                                            ...
                                            >
                                                ...
                                                <div className="flex-1 overflow-hidden relative">
                                                    <p className="text-base md:text-xl text-white/70 whitespace-pre-wrap leading-relaxed mb-3 md:mb-4">
                                                        {getTruncatedContent(note.content)}
                                                    </p>
            ...
            <button
                className="fixed md:absolute bottom-6 right-6 md:bottom-10 md:right-10 w-16 h-16 md:w-20 md:h-20 bg-white text-[#89216b] rounded-full shadow-2xl flex items-center justify-center hover:scale-110 md:hover:rotate-90 transition-all duration-300 z-40 group"
                onClick={addNote}
            >
                <FiPlus size={window.innerWidth < 768 ? 28 : 40} />
                ...
                >
                    ...
                    >
                        ...
                        <input
                            ...
                            className="bg-transparent border-none outline-none text-2xl md:text-5xl font-black text-white placeholder-white/30 mb-4 md:mb-8 w-full mt-12 md:mt-0"
                        />
                        ...
                        <textarea
                            ...
                            className="bg-transparent border-none outline-none flex-1 text-lg md:text-2xl text-white/80 placeholder-white/20 resize-none font-medium leading-relaxed overflow-auto [&::-webkit-scrollbar]:hidden"
                        ></textarea>
