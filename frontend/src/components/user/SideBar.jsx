...
import { FiSearch, FiPlus, FiFilter, FiCalendar, FiArchive, FiTrash2, FiSettings, FiHelpCircle, FiFileText, FiEdit3, FiChevronDown, FiX } from 'react-icons/fi';
...
const SideBar = ({ activeView, setActiveView, onSearch, onFilter, currentFilter, labels = [], setLabels, setSelectedLabel, selectedLabel, setNotes, onCreateLabel, onUpdateLabel, onDeleteLabel, filterDate, onDateFilter, settings, isOpen, onClose }) => {
    ...
    const handleUpdateLabel = (id, oldName) => {
        ...
        if (labels.some(l => l.name === newName && (l.id || l._id) !== id)) {
    ...
    return (
        ...
        `}>
            ...
            <div className="md:hidden flex items-center justify-between mb-8">
                <h1 className="text-3xl font-black tracking-tighter uppercase italic">Slate</h1>
            ...
            {/* Filter Button */}
            <div className="relative mb-4 md:mb-0" ref={filterRef}>
                <div
                ...
                >
                    ...
                    <span className="md:hidden text-lg font-bold uppercase tracking-wider">Filter Options</span>
                </div>

                {isFilterOpen && (
                    <div className={`absolute top-full left-0 mt-4 w-[calc(100vw-40px)] md:w-96 border border-white/20 rounded-2xl shadow-2xl p-6 z-[60] animate-fade-in origin-top-left flex flex-col gap-6 ${settings?.theme === 'dark' ? 'bg-[#1e1e2f]' : 'bg-[#89216b] md:bg-red-gradient'}`}>
            ...
            <div className="flex flex-col gap-4 mb-4">
                {labels && labels.map((labelObj, index) => {
                    ...
                    const labelId = labelObj.id || labelObj._id || index; // Fallback index if no ID (shouldn't happen with API)
                    ...
                    return (
                        <div
                            ...
                            onClick={() => {
                                setSelectedLabel && setSelectedLabel(labelName);
                                setActiveView('label');
